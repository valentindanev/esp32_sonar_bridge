...
#define DB_PARAM_NAME_MAXLEN                                                   \
  16 // max len of a parameter/key stored in the ESP32 NVM
#define DB_PARAM_MAX_MAV_PARAM_NAME_LEN                                        \
  16 // max len of the field used to store the mav param name (max len 16 by
     // def.)
#define DB_PARAM_VALUE_MAXLEN                                                  \
  64 // max len of a value stored for a key in the ESP32 NVM (string len)
...
// initially set pins to 0 to allow the start of the system on all boards. User
// has to set the correct pins
...
#define DB_PARAM_STA_IP_NETMASK                                                \
  db_param_wifi_sta_netmask.value.db_param_str.value
...
#define DB_PARAM_DIS_RADIO_ON_ARM                                              \
  db_param_dis_radio_armed.value.db_param_u8.value
...
#define DB_PARAM_SERIAL_PACK_SIZE                                              \
  db_param_serial_pack_size.value.db_param_u16.value
...
enum db_param_type_t { STRING, UINT8, UINT16, INT32 };

typedef struct db_parameter_s {
  uint8_t db_name[DB_PARAM_NAME_MAXLEN]; // equals the NVS key, JSON object name
                                         // and web interface input ID
  enum db_param_type_t type;
  struct { // no string support for mavlink
    uint8_t param_name[DB_PARAM_MAX_MAV_PARAM_NAME_LEN];
    uint16_t param_index; // strings will be ignored for mavlink set to -1 for
                          // strings
...
bool db_param_is_valid_u16(uint16_t new_u16_value,
                           db_parameter_t *target_param);
...
bool db_param_is_valid_assign_str(char *new_string_value,
                                  db_parameter_t *target_param);
bool db_param_is_valid_assign_u8(uint8_t new_u8_value,
                                 db_parameter_t *target_param);
bool db_param_is_valid_assign_u16(uint16_t new_u16_value,
                                  db_parameter_t *target_param);
bool db_param_is_valid_assign_i32(int32_t new_i32_value,
                                  db_parameter_t *target_param);
