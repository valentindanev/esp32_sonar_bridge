 ...
 * directly while system is live will eventually lead to crashes. Stores the new
 * Wi-Fi mode and will be written to the settings. This means it will become
 * active after reboot.
 */
...
db_parameter_t db_param_ssid, db_param_pass, db_param_wifi_ap_ip,
    ...
    db_param_udp_client_ip, db_param_wifi_hostname, db_param_deeper_ssid,
    db_param_deeper_pass = {0};

/* ---------- From here with increasing param_index all parameters that are also
 * available via MAVLink ---------- */
 ...
 *  disabled: only 802.11b support for client mode - set to true: 802.11b/g/n/ax
 * mode support. ax only when chip supports it.
 */
 ...
 * (MissionPlanner)
 */
db_parameter_t db_param_rssi_dbm = {.db_name = "rep_rssi_dbm",
                                    .type = UINT8,
                                    .mav_t =
                                        {
 ...
 */
db_parameter_t db_param_init_str_param(char *db_name, char *mav_param_name,
  ...
  if (mav_param_name_str_len < DB_PARAM_MAX_MAV_PARAM_NAME_LEN) {
    ...
    strncpy((char *)db_str_param.mav_t.param_name, mav_param_name,
            DB_PARAM_MAX_MAV_PARAM_NAME_LEN - 1);
  } else {
    // mav param name is allowed to be 16 chars long - it is valid without a
    // string terminator if len = 16 copy the first 16 chars of string without a
    // string terminator
    memcpy((char *)db_str_param.mav_t.param_name, mav_param_name,
  ...
  if (db_str_param.value.db_param_str.value == NULL) {
    ESP_LOGE(TAG, "Error allocating %i bytes for string parameter %s",
             max_val_len, db_name);
  } else {
    // all good - init value to default value
    strncpy((char *)db_str_param.value.db_param_str.value,
            (char *)db_str_param.value.db_param_str.default_value, max_val_len);
    ...
    ESP_LOGD(TAG, "Initialized value for %s with default '%s'", db_name,
...
/**
 * Initializes the parameters used for storing user settings. All parameters are
 * collected in an array. Leaks memory. Only call once during startup! Add new
 * parameters here!
 */
void db_param_init_parameters() {
  ...
  // auto IP.
  db_param_wifi_sta_ip = db_param_init_str_param("ip_sta", "WIFI_STA_IP", "", 0,
                                                 IP4ADDR_STRLEN_MAX);
  // If db_param_wifi_sta_ip is set then this must be set to the gateway IP
  db_param_wifi_sta_gw = db_param_init_str_param("ip_sta_gw", "WIFI_STA_GW", "",
                                                 0, IP4ADDR_STRLEN_MAX);
  ...
  // Users can add custom UDP client targets. This is the IP of the first target
  // added. Only the first one is saved to NVM.
  ...
  // Deeper Sonar SSID to connect to when Fallback is Enabled
  db_param_deeper_ssid = db_param_init_str_param("ss_deeper_ssid", "SS_DEEPER",
                                                 "Deeper CHIRP+ 3B6D", 1, 32);
  // Optional Deeper Sonar password. Leave empty when the Deeper AP is open.
  db_param_deeper_pass = db_param_init_str_param("ss_deeper_pass",
                                                 "SS_DEEPPASS", "", 0, 64);
  ...
  if (err == ESP_OK) {
    if (required_size > db_param->value.db_param_str.max_len) {
      ESP_LOGW(TAG, "NVS string for %s too long (%d > %d), using default.",
               (char *)db_param->db_name, required_size,
      ...
      // password)
      ESP_LOGI(TAG, "NVS string for %s is empty, using default.",
               (char *)db_param->db_name);
      db_param_set_to_default(db_param);
    } else {
      // Read the actual value
      err = nvs_get_str(my_handle, (char *)db_param->db_name,
                        (char *)db_param->value.db_param_str.value,
                        &required_size);
      if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error (%s) reading string %s from NVS! Using default.",
                 esp_err_to_name(err), (char *)db_param->db_name);
        db_param_set_to_default(db_param);
      } else {
    ...
    ESP_LOGI(TAG, "Parameter %s not found in NVS, using default.",
             (char *)db_param->db_name);
    db_param_set_to_default(db_param); // Apply the default value
  } else {
    ESP_LOGE(TAG, "Error (%s) checking NVS for %s! Using default.",
             esp_err_to_name(err), (char *)db_param->db_name);
  ...
  for (int i = 0; i < sizeof(db_params) / sizeof(db_params[0]); i++) {
    ...
    case UINT8:
      if (strcmp((char *)db_params[i]->db_name,
        ...
        ESP_ERROR_CHECK(nvs_set_u8(*nvs_handle, (char *)db_params[i]->db_name,
 ...
 * @return true if valid or false if not
 */
bool db_param_is_valid_u8(const uint8_t new_u8_value,
                          db_parameter_t *target_param) {
 ...
 * @return true if valid or false if not
 */
bool db_param_is_valid_u16(const uint16_t new_u16_value,
                           db_parameter_t *target_param) {
 ...
 * @return true if valid or false if not
 */
bool db_param_is_valid_i32(const int32_t new_i32_value,
                           db_parameter_t *target_param) {
 ...
 */
bool db_param_is_valid_assign_str(char *new_string_value,
                                  db_parameter_t *target_param) {
  if (db_param_is_valid_str(new_string_value, target_param)) {
    size_t max_len = target_param->value.db_param_str.max_len;
    strncpy((char *)target_param->value.db_param_str.value, new_string_value,
            max_len - 1);
    target_param->value.db_param_str.value[max_len - 1] = '\0';
    return true;
  } else {
    // new value is not valid - do not assign
    ESP_LOGE(TAG,
             "db_param_is_valid_assign_str(): Invalid string length (%i-%i) or "
             "NULL for param %s",
 ...
 * @return true when valid and assigned - else false
 */
bool db_param_is_valid_assign_u8(const uint8_t new_u8_value,
                                 db_parameter_t *target_param) {
  if (db_param_is_valid_u8(new_u8_value, target_param)) {
    if (strcmp((char *)target_param->db_name,
               (char *)db_param_radio_mode.db_name) == 0) {
      // Special case check: Do not directly change DB_WIFI_MODE since it is not
      // safe and constantly processed by other tasks. Save settings and reboot
      // will assign DB_RADIO_MODE_DESIGNATED to DB_WIFI_MODE
  ...
  ESP_LOGE(TAG,
           ...
           "(%i-%i) for param %s",
           new_u8_value, target_param->value.db_param_u8.max,
           target_param->value.db_param_u8.min, (char *)target_param->db_name);
  return false;
}
 ...
 * @return true when valid and assigned - else false
 */
bool db_param_is_valid_assign_u16(const uint16_t new_u16_value,
                                  db_parameter_t *target_param) {
  ...
  ESP_LOGE(TAG,
           ...
           "(%i-%i) for param %s",
           new_u16_value, target_param->value.db_param_u16.max,
           target_param->value.db_param_u16.min, (char *)target_param->db_name);
  return false;
}
 ...
 * @return true when valid and assigned - else false
 */
bool db_param_is_valid_assign_i32(const int32_t new_i32_value,
                                  db_parameter_t *target_param) {
  ...
  ESP_LOGE(TAG,
           ...
           "(%li-%li) for param %s",
           new_i32_value, target_param->value.db_param_i32.max,
           target_param->value.db_param_i32.min, (char *)target_param->db_name);
  return false;
}
