...
//   uint8_t  fmav_parse_to_frame_buf(fmav_result_t* result, uint8_t* buf,
//   fmav_status_t* status, uint8_t c) uint8_t
//   fmav_check_frame_buf(fmav_result_t* result, uint8_t* buf) void
//   fmav_frame_buf_to_msg(fmav_message_t* msg, fmav_result_t* result, uint8_t*
//   buf) uint8_t  fmav_parse_and_check_to_frame_buf(fmav_result_t* result,
//   uint8_t* buf, fmav_status_t* status, uint8_t c) uint8_t
//   fmav_parse_to_msg_wbuf(fmav_message_t* msg, uint8_t* buf, fmav_status_t*
//   status, uint8_t c)
//
// receive into msg:
//   uint8_t  fmav_parse_to_msg(fmav_message_t* msg, fmav_status_t* status,
//   uint8_t c)
//
// send from msg:
//   uint16_t fmav_msg_to_frame_buf(uint8_t* buf, fmav_message_t* msg)
//   uint16_t fmav_msg_to_frame_buf_wresult(fmav_result_t* result, uint8_t* buf,
//   fmav_message_t* msg) uint16_t fmav_msg_to_serial(fmav_message_t* msg)
//
// helper:
...
//   uint8_t  fmav_msg_is_for_me(uint8_t my_sysid, uint8_t my_compid,
//   fmav_message_t* msg) uint8_t  fmav_msg_result_is_for_me(uint8_t my_sysid,
//   uint8_t my_compid, fmav_result_t* result) void
//   fmav_status_reset_rx(fmav_status_t* status) void
//   fmav_status_reset_tx(fmav_status_t* status) void
//   fmav_status_reset(fmav_status_t* status) void     fmav_init(void)
//------------------------------
...
// uint8_t  fmav_payload_len_wo_trailing_zeros(const uint8_t* payload, uint8_t
// len) uint16_t fmav_finalize_msg(fmav_message_t* msg, fmav_status_t* status)
// uint16_t fmav_finalize_frame_buf(
//              uint8_t* buf, uint8_t payload_max_len, uint8_t crc_extra,
//              fmav_status_t* status)
// uint16_t fmav_finalize_serial(
//              uint8_t sysid, uint8_t compid, uint8_t* payload,
//              uint32_t msgid, uint8_t payload_max_len, uint8_t crc_extra,
//              fmav_status_t* status)
// uint8_t  fmav_check_msg(fmav_message_t* msg, fmav_status_t* status)
// void     fmav_parse_reset(fmav_status_t* status)

static const fmav_message_entry_t _fmav_message_crcs[] =
    FASTMAVLINK_MESSAGE_CRCS;

// this function is taken from the pymavlink-mavgen project
// https://github.com/ArduPilot/pymavlink/tree/master/generator
FASTMAVLINK_FUNCTION_DECORATOR const fmav_message_entry_t *
fmav_get_message_entry(uint32_t msgid) {
  // static const fmav_message_entry_t _fmav_message_crcs[] =
  // FASTMAVLINK_MESSAGE_CRCS;
  uint32_t low = 0;
  uint32_t high =
      sizeof(_fmav_message_crcs) / sizeof(_fmav_message_crcs[0]) - 1;
...
FASTMAVLINK_FUNCTION_DECORATOR const fmav_message_entry_t *
fmav_get_message_entry_by_index(uint32_t i) {
  return &(_fmav_message_crcs[i]);
}

// this function is taken from the pymavlink-mavgen project
// https://github.com/ArduPilot/pymavlink/tree/master/generator
FASTMAVLINK_FUNCTION_DECORATOR uint8_t
fmav_payload_len_wo_trailing_zeros(const uint8_t *payload, uint8_t len) {
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t
fmav_finalize_msg(fmav_message_t *msg, fmav_status_t *status) {
  uint8_t headbuf[FASTMAVLINK_HEADER_V2_LEN];

  headbuf[0] = msg->magic = FASTMAVLINK_MAGIC_V2;
  headbuf[1] = msg->len = fmav_payload_len_wo_trailing_zeros(
      msg->payload,
      msg->payload_max_len); // msg->payload_max_len was set in generator
  ...
  uint16_t crc =
      fmav_crc_calculate(&(headbuf[1]), FASTMAVLINK_HEADER_V2_LEN - 1);
  fmav_crc_accumulate_buf(&crc, msg->payload, msg->len);
  fmav_crc_accumulate(&crc,
                      msg->crc_extra); // msg->crc_extra was set in generator
  msg->checksum = crc;
  ...
  return (uint16_t)msg->len + FASTMAVLINK_HEADER_V2_LEN +
         FASTMAVLINK_CHECKSUM_LEN;
}

// used in message generators to finalize the frame buf
FASTMAVLINK_FUNCTION_DECORATOR uint16_t
fmav_finalize_frame_buf(uint8_t *buf, uint8_t payload_max_len,
                        uint8_t crc_extra, fmav_status_t *status) {
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t fmav_finalize_serial(
    uint8_t sysid, uint8_t compid, uint8_t *payload, uint32_t msgid,
    uint8_t payload_max_len, uint8_t crc_extra, fmav_status_t *status) {
  uint16_t len = fmav_payload_len_wo_trailing_zeros(payload, payload_max_len);
  uint16_t crc;
  fmav_crc_init(&crc);

  fmav_serial_write_char(FASTMAVLINK_MAGIC_V2);
  fmav_serial_write_char(len);
  fmav_crc_accumulate(&crc, len);
  fmav_serial_write_char(0);
  fmav_crc_accumulate(&crc, 0);
  fmav_serial_write_char(0);
  fmav_crc_accumulate(&crc, 0);
  fmav_serial_write_char(status->tx_seq);
  fmav_crc_accumulate(&crc, status->tx_seq);
  status->tx_seq++;
  fmav_serial_write_char(sysid);
  fmav_crc_accumulate(&crc, sysid);
  fmav_serial_write_char(compid);
  fmav_crc_accumulate(&crc, compid);
  fmav_serial_write_char(msgid);
  fmav_crc_accumulate(&crc, msgid);
  fmav_serial_write_char(msgid >> 8);
  fmav_crc_accumulate(&crc, msgid >> 8);
  fmav_serial_write_char(msgid >> 16);
  fmav_crc_accumulate(&crc, msgid >> 16);

  for (uint16_t i = 0; i < len; i++) {
    fmav_serial_write_char(payload[i]);
    fmav_crc_accumulate(&crc, payload[i]);
  }
...
// msg payload will be zero-filled, so that fmav_parse_to_msg() returns proper
// msg the checks are only possible if msgid is known, and also the targets are
// only then known, else we need to assume broadcast
FASTMAVLINK_FUNCTION_DECORATOR uint8_t fmav_check_msg(fmav_message_t *msg,
                                                      fmav_status_t *status) {
  msg->target_sysid = 0;
  msg->target_compid = 0;
  ...
  // check length
  // note: msg->len can be larger than PAYLOAD_LEN_MAX if the message has
  // unknown extensions
...
//------------------------------
// uint8_t  fmav_parse_to_frame_buf(fmav_result_t* result, uint8_t* buf,
// fmav_status_t* status, uint8_t c) uint8_t fmav_check_frame_buf(fmav_result_t*
// result, uint8_t* buf) void     fmav_frame_buf_to_msg(fmav_message_t* msg,
// fmav_result_t* result, uint8_t* buf) uint8_t
// fmav_parse_and_check_to_frame_buf(fmav_result_t* result, uint8_t* buf,
// fmav_status_t* status, uint8_t c) uint8_t
// fmav_parse_to_msg_wbuf(fmav_message_t* msg, uint8_t* buf, fmav_status_t*
// status, uint8_t c) uint8_t  fmav_parse_to_msg(fmav_message_t* msg,
// fmav_status_t* status, uint8_t c)

// returns NONE, HAS_HEADER, or OK
FASTMAVLINK_FUNCTION_DECORATOR uint8_t fmav_parse_to_frame_buf(
    fmav_result_t *result, uint8_t *buf, fmav_status_t *status, uint8_t c) {
  if (status->rx_cnt >=
      FASTMAVLINK_FRAME_LEN_MAX) { // this should never happen, but play it safe
    status->rx_state = FASTMAVLINK_PARSE_STATE_IDLE;
  }
  ...
  case FASTMAVLINK_PARSE_STATE_LEN:
    ...
    if (buf[0] == FASTMAVLINK_MAGIC_V2) {
      ...
      status->rx_frame_len =
          (uint16_t)c + FASTMAVLINK_HEADER_V2_LEN + FASTMAVLINK_CHECKSUM_LEN;
      status->rx_state = FASTMAVLINK_PARSE_STATE_INCOMPAT_FLAGS;
    } else {
      status->rx_header_len = FASTMAVLINK_HEADER_V1_LEN;
      status->rx_frame_len =
          (uint16_t)c + FASTMAVLINK_HEADER_V1_LEN + FASTMAVLINK_CHECKSUM_LEN;
  ...
  case FASTMAVLINK_PARSE_STATE_INCOMPAT_FLAGS:
    ...
    if (c != 0 && c != FASTMAVLINK_INCOMPAT_FLAGS_SIGNED) {
      status->rx_state =
          FASTMAVLINK_PARSE_STATE_IDLE; // incorrect incompat flag, so reset
    }
    result->res = FASTMAVLINK_PARSE_RESULT_NONE;
    return FASTMAVLINK_PARSE_RESULT_NONE;
  ...
  case FASTMAVLINK_PARSE_STATE_COMPAT_FLAGS:
    ...
    if (c != 0) {
      status->rx_state =
          FASTMAVLINK_PARSE_STATE_IDLE; // incorrect compat flag, so reset
    }
    result->res = FASTMAVLINK_PARSE_RESULT_NONE;
    return FASTMAVLINK_PARSE_RESULT_NONE;
#endif
...
// returns MSGID_UNKNOWN, LENGTH_ERROR, CRC_ERROR, SIGNATURE_ERROR, or OK
// the checks are only possible if msgid is known, and also the targets are only
// then known, else we need to assume broadcast
FASTMAVLINK_FUNCTION_DECORATOR uint8_t
fmav_check_frame_buf(fmav_result_t *result, uint8_t *buf) {
  if (buf[0] == FASTMAVLINK_MAGIC_V2) {
    result->sysid = buf[5];
    result->compid = buf[6];
    result->msgid = buf[7] + ((uint32_t)buf[8] << 8) + ((uint32_t)buf[9] << 16);
    result->frame_len =
        (uint16_t)buf[1] + FASTMAVLINK_HEADER_V2_LEN + FASTMAVLINK_CHECKSUM_LEN;
    if (buf[2] & FASTMAVLINK_INCOMPAT_FLAGS_SIGNED)
      result->frame_len += FASTMAVLINK_SIGNATURE_LEN;
  } else {
    result->sysid = buf[3];
    result->compid = buf[4];
    result->msgid = buf[5];
    result->frame_len =
        (uint16_t)buf[1] + FASTMAVLINK_HEADER_V1_LEN + FASTMAVLINK_CHECKSUM_LEN;
  }
  ...
  uint16_t payload_pos = (buf[0] == FASTMAVLINK_MAGIC_V2)
                             ? FASTMAVLINK_HEADER_V2_LEN
                             : FASTMAVLINK_HEADER_V1_LEN;

  // check crc
  uint16_t crc_pos = payload_pos + buf[1];
  uint16_t crc_frame = fmav_crc_calculate(
      &(buf[1]), crc_pos - 1); // the magic byte is not crc'ed
  ...
  if (msg_entry->flags & FASTMAVLINK_MESSAGE_ENTRY_FLAGS_HAS_TARGET_COMPONENT) {
    ...
    if (buf[1] > msg_entry->target_component_ofs) {
      result->target_compid =
          buf[payload_pos + msg_entry->target_component_ofs];
    }
  }
...
FASTMAVLINK_FUNCTION_DECORATOR void fmav_frame_buf_to_msg(fmav_message_t *msg,
  ...
  if (msg->len < result->payload_max_len) {
    memset(&(msg->payload[msg->len]), 0,
           result->payload_max_len - msg->len); // zero-fill
  }
  pos += msg->len;
...
// convenience wrapper
// returns 0, or 1
FASTMAVLINK_FUNCTION_DECORATOR uint8_t fmav_parse_and_check_to_frame_buf(
    fmav_result_t *result, uint8_t *buf, fmav_status_t *status, uint8_t c) {
  uint8_t res;
  ...
  // result can be MSGID_UNKNOWN, LENGTH_ERROR, CRC_ERROR, SIGNATURE_ERROR, or
  // OK
  if (res == FASTMAVLINK_PARSE_RESULT_MSGID_UNKNOWN ||
      res == FASTMAVLINK_PARSE_RESULT_OK) {
    return 1;
  }
...
// returns 0, or 1
// msg payload is zero-filled
FASTMAVLINK_FUNCTION_DECORATOR uint8_t fmav_parse_to_msg_wbuf(
    fmav_message_t *msg, uint8_t *buf, fmav_status_t *status, uint8_t c) {
  uint8_t res;
  fmav_result_t result;
...
// returns NONE, HAS_HEADER, MSGID_UNKNOWN, LENGTH_ERROR, CRC_ERROR,
// SIGNATURE_ERROR, or OK msg payload is zero-filled
FASTMAVLINK_FUNCTION_DECORATOR uint8_t fmav_parse_to_msg(fmav_message_t *msg,
                                                         fmav_status_t *status,
                                                         uint8_t c) {
  if (status->rx_cnt >=
      FASTMAVLINK_FRAME_LEN_MAX) { // this should never happen, but play it safe
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t
fmav_msg_to_frame_buf(uint8_t *buf, fmav_message_t *msg) {
  uint16_t pos = 0;
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t fmav_msg_to_frame_buf_wresult(
    fmav_result_t *result, uint8_t *buf, fmav_message_t *msg) {
...
FASTMAVLINK_FUNCTION_DECORATOR
...
// uint8_t  fmav_msg_is_for_me(uint8_t my_sysid, uint8_t my_compid,
// fmav_message_t* msg) uint8_t  fmav_msg_result_is_for_me(uint8_t my_sysid,
// uint8_t my_compid, fmav_result_t* result)
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t
fmav_msg_frame_len(fmav_message_t *msg) {
  return (uint16_t)msg->len +
         ((msg->magic == FASTMAVLINK_MAGIC_V2) ? FASTMAVLINK_HEADER_V2_LEN
                                               : FASTMAVLINK_HEADER_V1_LEN) +
         FASTMAVLINK_CHECKSUM_LEN +
         ((msg->incompat_flags & FASTMAVLINK_INCOMPAT_FLAGS_SIGNED)
              ? FASTMAVLINK_SIGNATURE_LEN
              : 0);
}
...
FASTMAVLINK_FUNCTION_DECORATOR uint8_t fmav_msg_result_is_for_me(
    uint8_t my_sysid, uint8_t my_compid, fmav_result_t *result) {
  if (result->target_sysid == 0)
    return 1;
  if (result->target_sysid != my_sysid)
    return 0;
  if (result->target_compid == 0)
    return 1;
  if (result->target_compid == my_compid)
    return 1;
  return 0;
}
...
FASTMAVLINK_FUNCTION_DECORATOR void
fmav_status_reset_rx(fmav_status_t *status) {
  status->rx_state = FASTMAVLINK_PARSE_STATE_IDLE;
  status->rx_cnt = 0;
  status->rx_header_len = 0;
  status->rx_frame_len = 0;
...
FASTMAVLINK_FUNCTION_DECORATOR void
fmav_status_reset_tx(fmav_status_t *status) {
  status->tx_seq = 0;
}
...
// call it once before using the library, even if it does not do anything
// currently
